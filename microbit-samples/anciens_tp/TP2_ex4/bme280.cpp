...
/* Sensor config
 * Performs configuration of the sensor and recovers calibration data from
 * sensor's internal memory. Return value: Upon successfull completion, returns
 * 0. On error, returns a negative integer equivalent to errors from glibc.
 */
#define CONF_BUF_SIZE 6
bme280::bme280(MicroBit *uB, MicroBitI2C *uBi2c, uint8_t addr,
               uint8_t humidity_os, uint8_t temp_os, uint8_t pressure_os,
               uint8_t sensor_mode, uint8_t standby, uint8_t coeff)
    : uBit(uB), i2c(uBi2c), address(addr), probe_ok(0),
      humidity_oversampling(humidity_os), temp_oversampling(temp_os),
      pressure_oversampling(pressure_os), mode(sensor_mode),
      standby_len(standby), filter_coeff(coeff) {
  int ret = 0;
  char cmd_buf[CONF_BUF_SIZE] = {
      BME280_REGS(ctrl_humidity),
      (uint8_t)BME280_CTRL_HUM(humidity_oversampling),
      BME280_REGS(ctrl_measure),
      (uint8_t)BME280_CTRL_MEA(pressure_oversampling, temp_oversampling, mode),
      BME280_REGS(config),
      (uint8_t)BME280_CONFIG(standby_len, filter_coeff),
  };
 ...
 * These values are required to compute the pressure, temperature and humidity
 * values from the uncompensated "raw" values read from the sensor ADC result
 * registers. Calibration data for pressure and temperature is aligned, packed,
 * and in little endian byte order, so it is stored directly to the cal
 * structure. Calibration data for humidity is split among different registers
 * and not aligned, so we use a temporary data buffer. Note : On the LPC822 the
 * I2C bus requires some time to go idle, so we need to sleep some time between
 * two conscutive I2C access. this should be done in the I2C driver, but is not
 * yet implemented, so it is done here. These msleep() calls may be removed for
 * other micro-controllers, and when the I2C driver for the LPC822 is imporved.
 * Return value:
 *   Upon successfull completion, returns 0. On error, returns a negative
...
int bme280::get_calibration_data() {
  ...
  uBit->sleep(1);
  /* Easy part : Temperature and Presure calibration data is packed, aligned,
  ...
  /* Read humidity calibration data. This one is split among bytes and not
   * aligned. use temporary data buffer and then copy to calibration structure.
   */
  /* First part */
 ...
 * Return value(s):
 *   Upon successfull completion, returns 0 and the raw sensor values read are
 * placed in the provided integer(s). On error, returns a negative integer
 ...
 * Returns the value in Pascal(Pa) or 0 on error (invalid value which would
 * cause division by 0). Output value of "96386" equals 96386 Pa = 963.86 hPa =
 * 963.86 millibar
 */
...
uint32_t bme280::compensate_humidity(int uncomp_humidity) {
  ...
  /* Calculate tmp2 */
  tmp2 = ((uncomp_humidity << 14) - (cal.H4 << 20) - (cal.H5 * tmp1) + 16384) >>
         15;
  /* Calculate tmp3 */
  tmp3 = ((((tmp1 * cal.H6) >> 10) * (((tmp1 * (int)cal.H3) >> 11) + 32768)) >>
          10) +
         2097152;
  /* Update tmp1 */
  ...
  /* Convert from 32bit integer in Q22.10 format (22 integer 10 fractional bits)
   * to a value in 0.01 %rH : A value of 42313 represents 42313 / 1024 = 41.321
   * %rH, convert it to 4132, which is 41.32 %rH.
   */
  humidity =
      ((humidity >> 10) * 100) + ((((humidity & 0x3FF) * 1000) >> 10) / 10);
  return humidity;
}
