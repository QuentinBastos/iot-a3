...
/**
 * This code is mainly an adaptation of code provided by Techno-Innov (Nathael
 * Pajani <nathael.pajani@ed3l.fr>) www.techno-innov.fr
...
#define BME280_CAL_REGS(x)                                                     \
  (((uint8_t)offsetof(struct bme280_calibration_regs, x)) + 0x88)
...
#define BME280_REGS(x)                                                         \
  (((uint8_t)offsetof(struct bme280_internal_regs, x)) + 0xD0)
...
#define BME280_CONFIG(stb, filt)                                               \
  ((((stb) & 0x07) << 5) | (((filt) & 0x07) << 2))

/* Data helpers */
#define BME280_DATA_20(msb, lsb, xlsb)                                         \
  ((((msb) & 0xFF) << 12) | (((lsb) & 0xFF) << 4) | (((xlsb) & 0xF0) >> 4))
...
public:
   ...
   * sensor's internal memory.
   */
  bme280(MicroBit *uB, MicroBitI2C *uBi2c, uint8_t addr = BME280_ADDR,
         uint8_t humidity_oversampling = BME280_OS_x16,
         uint8_t temp_oversampling = BME280_OS_x16,
         uint8_t pressure_oversampling = BME280_OS_x16,
         uint8_t mode = BME280_NORMAL, uint8_t standby_len = BME280_SB_62ms,
         uint8_t filter_coeff = BME280_FILT_OFF);
   ...
   * Param :
   *  - conf : bme280_sensor_configuration structure, with calibration data read
   * from sensor
